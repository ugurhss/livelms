...
Route::get('/courses/{courseId}/quizzes/{quizId}/results/{attemptId}', [QuizController::class, 'showResult'])->name('courses.quizzes.result');
Route::middleware(['auth'])->group(function () {
    // Quiz routes
    ...
    Route::get('/quizzes/{quiz}/edit', function ($quiz) {
        return view('quizzes.edit', ['quizId' => $quiz]);
    })->name('quizzes.edit');

    Route::get('/quizzes/{quiz}', function ($quiz) {
        return view('quizzes.show', ['quizId' => $quiz]);
    })->name('quizzes.show');
});
