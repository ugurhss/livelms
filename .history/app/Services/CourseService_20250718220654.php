   ...
   public function getCourseById(int $courseId): Course
{
    if (!is_numeric($courseId)) {
        throw new \InvalidArgumentException('Course ID must be a numeric value');
    }

    return $this->repository->getCourseById((int)$courseId);
}
    ...
    public function updateCourse(int $courseId, array $data): Course
    {
        ...
        if (isset($data['title'])) {
            $data['slug'] = $this->generateUniqueSlug($data['title'], $courseId);
        }
