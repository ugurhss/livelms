...
public function createCourseWithLessons(array $courseData, array $lessonsData): Course
{
    try {
        $courseData['user_id'] = Auth::id();
        $courseData['slug'] = $this->generateUniqueSlug($courseData['title']);

        return $this->repository->createWithLessons($courseData, $lessonsData);
    } catch (\Exception $e) {
        Log::error('Course creation with lessons failed: '.$e->getMessage());
        throw $e;
    }
}

public function updateCourseWithLessons(int $courseId, array $courseData, array $lessonsData): Course
{
    $this->validateCourseOwnership($courseId);

    if (isset($courseData['title'])) {
        $courseData['slug'] = $this->generateUniqueSlug($courseData['title'], $courseId);
    }

    return $this->repository->updateWithLessons($courseId, $courseData, $lessonsData);
}

public function getCourseWithLessons(int $courseId): Course
{
    return $this->repository->getCourseWithLessons($courseId);
}
}
