...
{
    Log::info('createCourse başlatıldı', ['data' => $data]);

    try {
        // Slug oluştur
        $data['slug'] = Str::slug($data['title']);
        $data['outcomes'] = json_encode($data['outcomes'] ?? []);
        $data['prerequisites'] = json_encode($data['prerequisites'] ?? []);
        $data['user_id'] = Auth::id();

        Log::info('Kurs verileri hazırlandı', $data);

        $course = $this->repository->createCourse($data);

        if (!$course) {
            Log::error('Kurs oluşturulamadı');
        } else {
            Log::info('Kurs oluşturuldu', ['course_id' => $course->id]);
        }
        ...
        if (!empty($data['lessons'])) {
            foreach ($data['lessons'] as $lessonData) {
                try {
                    ...
                    Log::info('Ders eklendi', $lessonData);
                } catch (\Exception $e) {
                    Log::error('Ders eklenemedi', [
                        'error' => $e->getMessage(),
                        'lesson' => $lessonData
                    ]);
                }
        ...
        Log::error('createCourse exception', [
            'message' => $e->getMessage(),
            'trace' => $e->getTraceAsString(),
        ]);
        throw $e; // hata fırlatılmaya devam edilir
    }}
