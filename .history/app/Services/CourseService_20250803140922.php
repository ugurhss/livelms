...
private function updateLessonsForCourse(Course $course, array $lessons): void
{
    $totalDuration = 0;
    ...
    foreach ($lessons as $lessonData) {
        ...
        if (isset($lessonData['id'])) {
            ...
            if ($lesson) {
                $slug = $this->generateUniqueLessonSlug($lessonData['title'], $course->id, $lessonData['id']);
                ...
                $lesson->update([
                    ...
                    'duration_minutes' => $lessonDuration,
                    'video_url' => $lessonData['video_url'] ?? null,
                    'order' => $lessonData['order'] ?? null,
                    'is_free' => $lessonData['is_free'] ?? false,
                ]);
                $updatedLessonIds[] = $lesson->id;
            }
        } else {
            // Add new lesson with unique slug
            $slug = $this->generateUniqueLessonSlug($lessonData['title'], $course->id);
   ...
   public function updateCourse(int $courseId, array $courseDetails): Course
{
    try {
        // Thumbnail işleme
        $thumbnailPath = $this->handleThumbnail($courseDetails['thumbnail'] ?? null);

        // Kurs verilerini hazırla
        $courseData = $this->prepareUpdateData($courseDetails, $thumbnailPath);

        // Kursu güncelle
        $course = $this->courseRepository->updateCourse($courseId, $courseData);

        // Dersleri güncelle (BU KISIM YENİ EKLENDİ)
        if (isset($courseDetails['lessons'])) {
            $this->updateLessonsForCourse($course, $courseDetails['lessons']);
        }
...
private function generateUniqueLessonSlug(string $title, int $courseId): string
{
    $slug = Str::slug($title);
    $count = Lesson::where('slug', $slug)
   ...
   public function enrollUser(int $courseId, int $userId): void
{
    ...
    if ($course->status !== 'published') {
        throw new RuntimeException('Bu kurs şu anda kayıt almıyor');
    }
...
//ilereme durumu
    ...
    public function getUserProgress(int $courseId, int $userId): int
{
    $totalLessons = Lesson::where('course_id', $courseId)->count();
    if ($totalLessons === 0) return 0;

    $completedLessons = LessonCompletion::whereHas('lesson', function($q) use ($courseId) {
...
public function updateUserProgress(int $courseId, int $userId): void
{
    $progress = $this->getUserProgress($courseId, $userId);
