...
public function store(StoreCourseRequest $request)
{
    ...
    Log::info('Kurs oluşturma işlemi başladı', [
        ...
        'request_data' => $request->except(['password', '_token']) // Hassas verileri loglamıyoruz
    ]);
    ...
    try {
        ...
        // Validasyon sonrası log
        Log::debug('Kurs verileri validasyondan geçti', [
            'validated_data' => $data,
            'validation_rules' => (new StoreCourseRequest())->rules()
        ]);

        $course = $this->courseService->createCourse($data);

        // Kurs oluşturma başarılı logu
        Log::notice('Kurs başarıyla oluşturuldu', [
            'course_id' => $course->id,
        ...
        // İşlem hatayla sonuçlandı logu
        Log::info('Kurs oluşturma işlemi hatayla sonuçlandı', [
            'execution_time' => microtime(true) - LARAVEL_START,
            'status' => 'failed'
        ]);
