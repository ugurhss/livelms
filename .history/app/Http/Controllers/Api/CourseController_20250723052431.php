   ...
   public function edit($id)
{
    ...
    $categories = \App\Models\Category::all(); // Veya kendi repository/service'iniz
    $levels = \App\Models\Level::all(); // Veya kendi repository/service'iniz
    return view('course.edit', compact('course', 'categories', 'levels'));
}
...
public function update(UpdateCourseRequest $request, $id)
    {
        try {
            ...
            // JSON alanları düzgün şekilde encode et
            if (isset($validated['outcomes'])) {
                $validated['outcomes'] = array_values(array_filter($validated['outcomes']));
            }

            if (isset($validated['prerequisites'])) {
                $validated['prerequisites'] = array_values(array_filter($validated['prerequisites']));
            }
