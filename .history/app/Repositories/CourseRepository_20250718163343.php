...
{
    Log::info('CourseRepository::createCourse çağrıldı', $courseDetails);

    try {
        $course = Course::create($courseDetails);

        Log::info('Course::create sonucu', ['id' => $course->id ?? null]);

        return $course;
    } catch (\Exception $e) {
        Log::error('Course oluşturulamadı', [
