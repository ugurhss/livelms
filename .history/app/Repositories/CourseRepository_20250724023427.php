...
public function createCourse(array $data): Course
{
    ...
    try {
        Log::info('Kurs oluşturma işlemi başladı', [
            'user_id' => Auth::id(),
            'data' => array_merge($data, ['thumbnail' => isset($data['thumbnail']) ? 'file_uploaded' : null])
        ]);
        ...
        if (isset($data['thumbnail']) && $data['thumbnail'] instanceof \Illuminate\Http\UploadedFile) {
            ...
            Log::debug('Thumbnail dosyası başarıyla yüklendi', [
                'path' => $thumbnailPath,
                'size' => $data['thumbnail']->getSize(),
                'mime_type' => $data['thumbnail']->getMimeType()
            ]);
        }
        ...
        Log::info('Kurs başarıyla oluşturuldu', [
            'course_id' => $course->id,
        ...
        if (isset($data['lessons']) && is_array($data['lessons'])) {
            Log::debug('Ders ekleme işlemi başladı', [
                'lesson_count' => count($data['lessons'])
            ]);
        ...
        Log::debug('Kurs süresi güncellendi', [
            'new_duration' => $totalDuration
        ]);

        DB::commit();

        Log::info('Kurs oluşturma işlemi başarıyla tamamlandı', [
            'course_id' => $course->id,
        ...
        Log::error('Kurs oluşturma hatası', [
            'user_id' => Auth::id(),
            'error_message' => $e->getMessage(),
            'error_trace' => $e->getTraceAsString(),
            'input_data' => array_merge($data, ['thumbnail' => isset($data['thumbnail']) ? 'file_uploaded' : null])
        ]);
