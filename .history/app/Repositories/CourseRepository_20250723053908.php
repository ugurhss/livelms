   ...
   public function checkEnrollment(int $courseId, int $userId): bool
{
    ...
    //********************************************************       */
public function createWithLessons(array $courseData, array $lessonsData)
    {
        return DB::transaction(function () use ($courseData, $lessonsData) {
            $course = $this->createCourse($courseData);

            foreach ($lessonsData as $index => $lessonData) {
                $this->createLesson($course->id, $lessonData, $index + 1);
            }
    ...
    public function updateWithLessons($id, array $courseData, array $lessonsData)
    {
        return DB::transaction(function () use ($id, $courseData, $lessonsData) {
            $course = $this->updateCourse($id, $courseData);
    ...
    protected function createCourse(array $data)
    {
        $data['user_id'] = auth()->id();
        $data['outcomes'] = $this->prepareArrayData($data['outcomes'] ?? []);
        $data['prerequisites'] = $this->prepareArrayData($data['prerequisites'] ?? []);
    ...
    protected function updateCourse($id, array $data)
    {
        $course = $this->find($id);
    ...
    protected function createLesson($courseId, array $data, $order)
    {
    ...
    protected function prepareArrayData(array $items)
    {
