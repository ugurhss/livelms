                        ...
                        <div class="flex items-center">
                            ...
                            @if($isEnrolled || $lesson->is_free)
                                <button
                                    ...
                                    @click="window.dispatchEvent(new CustomEvent('open-lesson-modal', { detail: { lessonId: {{ $lesson->id }} } }))"
...
<div id="lesson-player-modal" class="hs-overlay hidden size-full fixed top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto bg-gray-900" role="dialog" tabindex="-1">
    <div class="hs-overlay-open:opacity-100 hs-overlay-open:duration-500 opacity-0 ease-out transition-all size-full flex">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <div class="absolute top-4 end-4 z-10">
                <button type="button" class="size-8 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-800 text-gray-200 hover:bg-gray-700 focus:outline-none focus:bg-gray-700 disabled:opacity-50 disabled:pointer-events-none" data-hs-overlay="#lesson-player-modal">
            ...
            <div class="bg-gray-800 p-4 border-t border-gray-700">
                <h2 class="text-xl font-bold text-white">{{ $lesson->title ?? 'Ders Başlığı' }}</h2>
                <p class="text-gray-400 mt-1">{{ $lesson->description ?? 'Ders açıklaması' }}</p>
            </div>
        </div>
                ...
                @foreach($course->lessons as $courseLesson)
                    <div class="p-4">
                        <a
                            ...
                            class="flex items-center gap-3 p-2 rounded-lg {{ $lesson->id === $courseLesson->id ? 'bg-gray-700 text-white' : 'hover:bg-gray-700 text-gray-300 hover:text-white' }}"
                        >
                            <span class="shrink-0 size-6 flex items-center justify-center {{ $lesson->id === $courseLesson->id ? 'bg-blue-600' : 'border border-gray-600' }} rounded-full text-xs">
                                {{ $loop->iteration }}
                            </span>
                            <span>{{ $courseLesson->title }}</span>
                            @if($courseLesson->is_free)
                                <span class="ml-auto px-2 py-1 bg-green-900 text-green-100 text-xs rounded-full">Ücretsiz</span>
                            @endif
                        </a>
                    </div>
                @endforeach
    ...
    document.addEventListener('DOMContentLoaded', function() {
        window.addEventListener('open-lesson-modal', function(e) {
