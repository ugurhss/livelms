<x-layouts.app>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8 flex justify-between items-center">
            <h1 class="text-3xl font-bold text-gray-900">Yeni Kurs Oluştur</h1>
            <a href="{{ route('courses.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Kurslara Geri Dön
            </a>
        </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white shadow-sm rounded-lg p-6">
                        ...
                        <div class="space-y-6">
                            ...
                            <!-- Description -->
                            <div>
                                <label for="description" class="block text-sm font-medium text-gray-700">Kurs Açıklaması</h2>
                                <textarea name="description" id="description" rows="5" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">{{ old('description') }}</textarea>
                                @error('description')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                    ...
                    <div class="bg-white shadow-sm rounded-lg p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price', 0) }}" min="0" step="0.01" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                @error('price')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="original_price" id="original_price" value="{{ old('original_price', 0) }}" min="0" step="0.01" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <div>
                                ...
                                <select name="level" id="level" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500">
                                    <option value="beginner" {{ old('level') == 'beginner' ? 'selected' : '' }}>Başlangıç</option>
                                    <option value="intermediate" {{ old('level') == 'intermediate' ? 'selected' : '' }}>Orta Seviye</option>
                                    <option value="advanced" {{ old('level') == 'advanced' ? 'selected' : '' }}>İleri Seviye</option>
                                </select>
                                @error('level')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="bg-white shadow-sm rounded-lg p-6">
                        ...
                        <div class="space-y-6" id="lessons-container">
                            @foreach(old('lessons', []) as $index => $lesson)
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-white shadow-sm rounded-lg p-6 sticky top-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <a href="{{ route('courses.index') }}" class="w-full flex justify-center items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md shadow-sm text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                İptal
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="bg-white shadow-sm rounded-lg p-6">
                        ...
                        <div class="bg-gray-100 h-40 rounded-lg flex items-center justify-center text-gray-400 mb-4">
                            <span>Kurs resmi burada görünecek</span>
                        </div>
                        <h3 class="font-medium text-gray-900">{{ old('title', 'Kurs Başlığı') }}</h3>
                        <p class="text-sm text-gray-500 mt-1">
                            @php
                                $level = old('level', 'beginner');
                                $levelText = $level === 'beginner' ? 'Başlangıç' : ($level === 'intermediate' ? 'Orta Seviye' : 'İleri Seviye');
                            @endphp
                            {{ $levelText }} • <span id="lesson-count">0</span> ders
                        </p>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Update lesson count in preview
            function updateLessonCount() {
                const count = document.querySelectorAll('.lesson-item').length;
                document.getElementById('lesson-count').textContent = count;
            }
            ...
            document.getElementById('title').addEventListener('input', function() {
                document.querySelector('.lg\\:col-span-1 .font-medium.text-gray-900').textContent = this.value || 'Kurs Başlığı';
            });

            // Update preview when level changes
            document.getElementById('level').addEventListener('change', function() {
                const levelText = this.value === 'beginner' ? 'Başlangıç' :
                                this.value === 'intermediate' ? 'Orta Seviye' : 'İleri Seviye';
                document.querySelector('.lg\\:col-span-1 .text-sm.text-gray-500').innerHTML =
                    `${levelText} • <span id="lesson-count">${document.querySelectorAll('.lesson-item').length}</span> ders`;
            });
        });
    </script>
    @endpush
