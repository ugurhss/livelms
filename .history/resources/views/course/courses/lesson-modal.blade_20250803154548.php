<x-layouts.app>
    ...
    @if($currentLesson->video_type === 'youtube')
        <iframe class="w-full h-64 md:h-96"
                src="https://www.youtube.com/embed/{{ $currentLesson->video_id }}?modestbranding=1&rel=0&controls=1&disablekb=1&showinfo=0"
                frameborder="0"
...
document.getElementById('lessonVideo')?.addEventListener('contextmenu', function(e) {
    e.preventDefault();
    return false;
});
...
document.addEventListener('keydown', function(e) {
    if (e.target.id === 'lessonVideo') {
        // Space, Arrow keys, K, M gibi tuşları engelle
        if ([32, 37, 38, 39, 40, 75, 77].includes(e.keyCode)) {
            e.preventDefault();
        }
    }
});
