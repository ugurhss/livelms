    ...
    @if($course)
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="relative rounded-2xl overflow-hidden h-80 bg-gradient-to-r from-indigo-600 to-purple-600 mb-8">
                @if($course->thumbnail)
                    <img src="{{ $course->thumbnail }}" alt="{{ $course->title }}" class="absolute inset-0 w-full h-full object-cover mix-blend-overlay opacity-20" />
                @endif
                ...
                <div class="absolute bottom-0 left-0 p-8 text-white">
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                        <div class="flex items-center">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="w-4 h-4 {{ $i <= ($course->reviews_avg_rating ?? 0) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                ...
                @if($isInstructor)
                    <div class="absolute top-6 right-6 flex space-x-3">
                        <a href="{{ route('courses.edit', $course->id) }}" class="p-2.5 bg-white/20 backdrop-blur-sm rounded-full text-white hover:bg-white/30 transition-colors" title="Kursu Düzenle">
                ...
                <div class="lg:col-span-2 space-y-8">
                            ...
                            <div class="divide-y divide-gray-200">
                                @foreach($course->lessons as $index => $lesson)
                                    <div class="accordion-item overflow-hidden" x-data="{ open: false }">
                                        <div @click="open = !open" class="accordion-header flex items-center justify-between p-6 cursor-pointer hover:bg-gray-50 transition-colors">
                                            <div class="flex items-center">
                                                <svg class="accordion-icon w-5 h-5 mr-3 text-gray-500 transition-transform duration-200" :class="open ? 'transform rotate-90 text-indigo-500' : ''" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <div class="accordion-content bg-gray-50 px-6" :class="open ? 'active' : ''" x-show="open" x-collapse>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-8">
                        ...
                        <div class="flex flex-col sm:flex-row gap-6">
                            <img src="{{ $course->instructor->profile_photo_url ?? asset('images/default-avatar.png') }}" alt="{{ $course->instructor->name }}" class="w-24 h-24 rounded-full object-cover" />
                            <div>
                                ...
                                @foreach($course->reviews as $review)
                                    <div class="border-b pb-6 last:border-b-0 last:pb-0">
                                        <div class="flex items-center mb-3">
                                            <img src="{{ $review->user->profile_photo_url ?? asset('images/default-avatar.png') }}" alt="{{ $review->user->name }}" class="w-10 h-10 rounded-full mr-3">
                                            <div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 sticky top-6">
                        <div class="text-center mb-6">
                            ...
                            @if($course->price && $course->original_price)
                                <p class="text-gray-500 line-through mt-1">₺{{ number_format($course->original_price, 2) }}</p>
                            @endif
                        </div>
                        @if(!$isEnrolled && !$isInstructor)
                            @if(Auth::check())
                                <form action="{{ route('courses.enroll', $course) }}" method="POST" id="enrollForm">
                                    @csrf
                                    <button type="submit" class="w-full flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200">
                                        Kursa Kayıt Ol
                                    </button>
                                </form>
                            @else
                                <a href="{{ route('login', ['redirect' => url()->current()]) }}" class="w-full flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200">
                                    Giriş Yap ve Kayıt Ol
                                </a>
                            @endif
                        @elseif($isEnrolled)
                            <div class="mb-4">
                                <div class="alert alert-success bg-green-100 text-green-800 p-3 rounded-md text-center">
                        ...
                        @else
                            <a href="{{ route('courses.edit', $course) }}" class="w-full flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200">
                                Kursu Düzenle
                            </a>
                        @endif
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 mt-6">
                        ...
                        <ul class="space-y-3">
                            ...
                            <li class="flex justify-between items-center">
                                ...
                                <span class="font-medium">{{ number_format($course->enrollments->avg('progress') ?? 0, 0) }}%</span>
                            </li>
                        </ul>
                    </div>
                </div>
...
@livewire('lesson-player', ['course' => $course, 'lessonId' => $lesson->id], key($lesson->id))    @if($isEnrolled || $course->lessons->contains('is_free', true))
        @livewire('lesson-player', ['course' => $course])
    @endif

    <!-- Alpine.js, Collapse Plugin ve Livewire Scripts -->
    <script src="https://cdn.jsdelivr.net/npm/alpinejs@3.x.x/dist/cdn.min.js" defer></script>
    <script src="https://cdn.jsdelivr.net/npm/@alpinejs/collapse@3.x.x/dist/cdn.min.js" defer></script>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Tüm accordion'ları aç/kapat
            ...
            // Livewire ile modal açıldığında sayfayı yukarı kaydır
