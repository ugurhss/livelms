<x-layouts.app>
                ...
                <div class="bg-white rounded-xl shadow-xs md:shadow-sm overflow-hidden border border-gray-100">
                    @if(request()->has('lesson'))
                        ...
                        <div class="p-4 md:p-6">
                            <!-- Video Player -->
                           <div class="aspect-w-16 aspect-h-9 mb-6 rounded-xl overflow-hidden bg-black relative">
    @if($currentLesson->video_type === 'youtube')
        <!-- YouTube gizli embed -->
        <iframe class="w-full h-64 md:h-96 opacity-0 absolute"
                src="https://www.youtube.com/embed/{{ $currentLesson->video_id }}?autoplay=1&controls=0&disablekb=1&fs=0&modestbranding=1&rel=0&showinfo=0&enablejsapi=1"
                frameborder="0"
                allow="autoplay; encrypted-media"
                id="hiddenYoutubePlayer"
                allowfullscreen>
        </iframe>

        <!-- Özel kontrol arayüzü -->
        <div class="relative w-full h-full bg-black" id="customPlayer">
            <div class="absolute inset-0 flex items-center justify-center">
                <button id="playButton" class="p-4 bg-black bg-opacity-50 rounded-full text-white">
            ...
            <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4">
                <div class="flex items-center justify-between">
                    <div class="flex space-x-3">
                        <button id="customPlayPause" class="text-white">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                            </svg>
                        </button>
                        <div class="text-white text-sm" id="timeDisplay">0:00 / 0:00</div>
                    </div>
                ...
                <input type="range" min="0" max="100" value="0" class="w-full mt-2 h-1.5 bg-gray-600 rounded-lg appearance-none cursor-pointer" id="progressBar">
            </div>
        </div>
    @else
    ...
    if (typeof YT !== 'undefined' && YT.Player) {
        window.player = new YT.Player('hiddenYoutubePlayer', {
            events: {
...
function onPlayerReady(event) {
    ...
    document.getElementById('playButton').addEventListener('click', () => {
        player.playVideo();
        document.getElementById('customPlayer').style.opacity = '0';
        document.getElementById('customPlayer').style.pointerEvents = 'none';
    });
...
// Player durum değişikliklerinde
...
document.getElementById('customPlayer').addEventListener('contextmenu', (e) => {
    e.preventDefault();
    return false;
});
    </script>
