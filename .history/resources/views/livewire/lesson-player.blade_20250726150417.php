<div x-data="{ isOpen: @entangle('showModal') }">
    ...
    <div class="space-y-4 mb-8">
        @foreach($lessons as $lesson)
            <button wire:click="openModal({{ $lesson->id }})"
                    @click="isOpen = true"
                    class="w-full p-4 text-left border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors shadow-sm">
            ...
            <div class="flex-1 bg-black flex items-center justify-center relative">
                @if($currentLesson->video_url)
                    @if($currentLesson->video_type === 'youtube' && $currentLesson->video_id)
                        <iframe src="https://www.youtube.com/embed/{{ $currentLesson->video_id }}?autoplay=1&rel=0"
                                class="w-full h-full"
                                frameborder="0"
                                allowfullscreen>
                        </iframe>
        ...
        @else
            <div class="flex-1 flex items-center justify-center">
                <div class="text-center p-8">
                    ...
                    <h3 class="text-xl font-medium mb-2">Ders Bilgisi Yüklenemedi</h3>
