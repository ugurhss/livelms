                ...
                <div class="flex-1 bg-black flex items-center justify-center relative">
                    @if($currentLesson && $currentLesson->video_url)
                        @php
                            ...
                            $videoType = $currentLesson->video_type ?? 'unknown';
                            $videoId = $currentLesson->video_id ?? null;

                            // Eğer video_type yoksa veya youtube ise ID'yi çıkar
                            if (empty($videoType) || $videoType === 'youtube') {
                                $videoId = $currentLesson->extractYouTubeId($videoUrl);
                            }
                        @endphp

                        @if($videoType === 'youtube' || (str_contains($videoUrl, 'youtube.com') || str_contains($videoUrl, 'youtu.be'))
                            <iframe
                        ...
                        @elseif($videoType === 'html5' || preg_match('/\.(mp4|webm|ogg)$/i', $videoUrl))
                        ...
                        @else
                            <div class="text-center p-8 text-white">
                                ...
                                <h3 class="text-xl font-medium mb-2">Desteklenmeyen Video Türü</h3>
                                <p class="text-gray-300">URL: {{ Str::limit($videoUrl, 50) }}</p>
                                <p class="text-gray-300">Tip: {{ $videoType }}</p>
                            </div>
                        @endif
                    @else
                        <div class="text-center p-8 text-white">
                ...
                <div class="border-t border-gray-200 bg-white">
                    <div class="p-6">
                        ...
                        <div class="mt-6 flex items-center space-x-4 text-sm text-gray-500">
                            ...
                            <span>•</span>
                            <span>
                                Video Türü: {{ $currentLesson->video_type ?? 'Belirtilmemiş' }}
                            </span>
                        </div>
                    </div>
                </div>
