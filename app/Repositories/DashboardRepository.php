...
class DashboardRepository implements DashboardRepositoryInterface
{
    ...
    public function calculateMonthlyRevenue(): float
    {
        ...
        return (float) Course::where(DB::raw("DATE_FORMAT(created_at, '%Y-%m')"), $currentMonth)
            ->sum('price');
    }
 ...
 public function getStudentEnrolledCourses(int $studentId): array
{
    ...
    ->toArray() ?: []; // Ensure array is returned
}
    ...
    public function getStudentRecentAnnouncements(int $studentId, int $limit = 5): array
    {
        ...
        return Announcement::whereHas('course.students', function ($query) use ($studentId) {
            $query->where('user_id', $studentId);
        })
            ->with(['course'])
            ->latest()
