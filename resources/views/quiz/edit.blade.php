...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Quiz Düzenle</h1>
        <p class="mt-2 text-lg text-gray-600">"{{ $quiz->title }}" quizini güncelleyin</p>
    </div>
    ...
    <form action="{{ route('quizzes.update', $quiz->id) }}" method="POST" id="quiz-form">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" id="title" name="title" value="{{ old('title', $quiz->title) }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>

                    <!-- Description -->
                    <div class="md:col-span-2">
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Açıklama</label>
                        <textarea id="description" name="description" rows="2" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description', $quiz->description) }}</textarea>
                    </div>

                    <!-- Settings -->
                    <div>
                        <label for="time_limit" class="block text-sm font-medium text-gray-700 mb-1">Süre Limiti (dakika)</label>
                        <input type="number" id="time_limit" name="time_limit" min="1" value="{{ old('time_limit', $quiz->time_limit) }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>

                    <div>
                        <label for="passing_score" class="block text-sm font-medium text-gray-700 mb-1">Geçme Notu (%)</label>
                        <input type="number" id="passing_score" name="passing_score" min="0" max="100" value="{{ old('passing_score', $quiz->passing_score) }}" required class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="is_published" name="is_published" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
                            <option value="0" {{ !$quiz->is_published ? 'selected' : '' }}>Taslak</option>
                            <option value="1" {{ $quiz->is_published ? 'selected' : '' }}>Yayında</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" id="start_date" name="start_date" value="{{ old('start_date', $quiz->start_date ? $quiz->start_date->format('Y-m-d\TH:i') : '') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>

                    <div>
                        <label for="end_date" class="block text-sm font-medium text-gray-700 mb-1">Bitiş Tarihi</label>
                        <input type="datetime-local" id="end_date" name="end_date" value="{{ old('end_date', $quiz->end_date ? $quiz->end_date->format('Y-m-d\TH:i') : '') }}" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                </div>
            </div>
        </div>
                        ...
                        <div class="question-item border rounded-lg overflow-hidden bg-white">
                            <div class="px-4 py-3 bg-gray-50 border-b flex justify-between items-center">
                                <h4 class="font-medium text-gray-900">
                                    <span class="question-number">{{ $questionIndex + 1 }}</span>. Soru
                                </h4>
                            ...
                            <div class="p-4">
                                <div class="mb-4">
                                    ...
                                    <textarea name="questions[{{ $questionIndex }}][question_text]" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>{{ old("questions.$questionIndex.question_text", $question->question_text) }}</textarea>
                                </div>

                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Soru Tipi</label>
                                        <select name="questions[{{ $questionIndex }}][question_type]" class="question-type w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                            <option value="multiple_choice" {{ $question->question_type == 'multiple_choice' ? 'selected' : '' }}>Çoktan Seçmeli</option>
                                            <option value="true_false" {{ $question->question_type == 'true_false' ? 'selected' : '' }}>Doğru/Yanlış</option>
                                            <option value="short_answer" {{ $question->question_type == 'short_answer' ? 'selected' : '' }}>Kısa Cevap</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Puan</label>
                                        <input type="number" name="questions[{{ $questionIndex }}][points]" min="1" value="{{ old("questions.$questionIndex.points", $question->points) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    </div>
                                </div>
                                    ...
                                    <div class="answers-list space-y-2">
                                        @foreach($question->answers as $answerIndex => $answer)
                                            <div class="answer-item flex items-center gap-3">
                                                <input type="text" name="questions[{{ $questionIndex }}][answers][{{ $answerIndex }}][answer_text]" value="{{ old("questions.$questionIndex.answers.$answerIndex.answer_text", $answer->answer_text) }}" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                                <div class="flex items-center">
                                                    <input type="checkbox" name="questions[{{ $questionIndex }}][answers][{{ $answerIndex }}][is_correct]" value="1" class="is-correct h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ $answer->is_correct ? 'checked' : '' }}>
        ...
        <div class="flex justify-end gap-3">
            {{-- <a href="{{ route('courses.quizzes.show', $quiz->id) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                İptal
            </a> --}}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize answer fields for existing questions
